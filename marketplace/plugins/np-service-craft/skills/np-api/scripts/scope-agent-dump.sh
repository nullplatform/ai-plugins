#!/bin/bash
#
# Dump K8s status for a scope via the runtime agent
#
# Usage:
#   ./scope-agent-dump.sh <scope_id> [nrn] [selector_key] [selector_value]
#
# Examples:
#   ./scope-agent-dump.sh 1566332102
#   ./scope-agent-dump.sh 1566332102 "organization=4:account=17"
#   ./scope-agent-dump.sh 331781883 "organization=1255165411:account=95118862" environment javi-k8s
#
# Output:
...
if [ $# -lt 1 ]; then
    echo "Usage: $0 <scope_id> [nrn] [selector_key] [selector_value]"
    echo ""
    echo "Dump K8s status for a scope via the runtime agent."
    echo ""
    echo "Arguments:"
    echo "  scope_id        - The scope ID to dump status for"
    ...
    echo "Examples:"
    echo "  $0 1566332102"
    echo "  $0 1566332102 \"organization=4:account=17\""
    echo "  $0 331781883 \"organization=1255165411:account=95118862\" environment javi-k8s"
    echo ""
    echo "Notes:"
    ...
    '{
        ...
        command: {
            ...
            data: {
                ...
                arguments: ["--scope-id", $scope_id, "--k8s-namespace", "nullplatform"]
            }
        }
    }')
