...
> Las transiciones de status se hacen solo con los endpoints de acción (`defer` / `reject` / `resolve` / `reopen` / `close`); cambiar `status` vía `PATCH` / `PUT` responde **403** (ver `lifecycle.md`). Las decisiones de aprobación las gestiona el servicio de aprobaciones de la plataforma.
