...
> Las transiciones de status se hacen solo con los endpoints de acción (`defer` / `reject` / `resolve` / `reopen` / `close`). Cambiar `status` vía `PATCH` / `PUT` requiere la capability interna `approval:bypass`, reservada al servicio de aprobaciones de la plataforma; no es un claim que un consumidor deba pedir (ver `lifecycle.md`). Las decisiones de aprobación llegan por callback del servicio de aprobaciones de la plataforma.
