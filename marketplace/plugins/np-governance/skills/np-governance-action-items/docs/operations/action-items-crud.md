...
> **Identidad**: la API resuelve la identidad del actor a partir del token. Los flags `--actor` / `--created-by` / `--author` son opcionales y solo se honran para llamadores con derechos de delegación (un agente actuando en nombre de un usuario); si no, se ignoran y se registra la identidad del token.
...
Solo desde `rejected` o `deferred`. Vuelve a `open`, limpia `resolved_at` y `deferred_until`. La API ignora el body (el `--actor` solo aplica a la atribución por delegación). Requiere el claim `governance:action_item:reopen`.
