...
- **Identidad**: la API resuelve la identidad del actor a partir del token; `--created-by` / `--actor` solo se honran para llamadores con derechos de delegación. El `created_by` con el que quedan los items creados debe coincidir con el `--agent-id` usado para filtrar en el próximo run. Si el token del agente **no** tiene delegación, los items quedan atribuidos a la identidad del token y el filtro `created_by=<agent-id>` no los va a encontrar en el siguiente run (y el reconciler recrearía duplicados). Recomendación práctica: usar como `--agent-id` la misma identidad del token, o correr con delegación.
