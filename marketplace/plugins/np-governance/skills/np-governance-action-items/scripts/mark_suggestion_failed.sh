#!/bin/bash
#
# mark_suggestion_failed.sh - Report a failed execution (retryable)
#
# Usage:
...
#
# Only valid from the "approved" state. Not terminal — can transition back
# to "approved" via retry_suggestion.sh.
...
DATA=$(jq -n --argjson er "$EXECUTION_RESULT" '{status: "failed", execution_result: $er}')
